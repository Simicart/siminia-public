...
const Robots = props => {
    ...
    if (['CATEGORY', 'PRODUCT'].includes(props.pageType) && robotsConfig) {
        ...
        if (props.pageType === 'CATEGORY' && filterVals) {
            ...
            if (
                count_filters_for_noindex &&
                parseInt(count_filters_for_noindex) <=
                    Object.keys(filterVals).length
            ) {
        ...
        noindex_nofollow_additional_pages &&
            noindex_nofollow_additional_pages.every(page => {
                if (actions.includes(page) && page) {
                    // is product page or category
                    if (
                        (page === 'catalog_category_view' &&
        ...
        if (props.pageType === 'CATEGORY') {
            attribute_combinations &&
                ...
                Object.keys(attribute_combinations).every(attrKey => {
                    if (Object.keys(filterVals).includes(attrKey)) {
                        // if exist attribute name in filter param
                        content = attribute_combinations[attrKey];
                        return false;
                    } else if (attrKey.split('+').length > 1) {
                        // example brand+color
                        if (
                            attrKey.split('+').every(key => {
                    ...
                    if (page.includes(pageKey)) {
                        return pages[pageKey].every(urlPath => {
                            if (
                                ...
                                props.location.pathname.includes(urlPath) &&
                                urlPath
                            ) {
                                content = 'noindex, follow';
