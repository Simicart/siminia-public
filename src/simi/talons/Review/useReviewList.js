...
export const useReviewList = props => {
    ...
    const [getReviewItems, { data: data, error: error, loading: loading }] = useLazyQuery(
        getProductReview,
    ...
    }, [sku, getReviewItems, currentPage, thisPage]);
    ...
    const dataReview = useMemo(() => {
        if (data && data.hasOwnProperty('productDetail') && data.productDetail && data.productDetail.items) {
            ...
            if (!loadedReview) {
                ...
                let loadedItems = loadedReview.productDetail.items[0].reviews.items;
                loadedItems = loadedItems.concat(items);
                for (var i = 0; i < loadedItems.length; ++i) {
                    for (var j = i + 1; j < loadedItems.length; ++j) {
                        if (loadedItems[i] && loadedItems[j] && loadedItems[i].nickname === loadedItems[j].nickname && loadedItems[i].summary === loadedItems[j].summary)
                            loadedItems.splice(j--, 1);
                    }
                }
                loadedReview.productDetail.items[0].reviews.items = loadedItems;
            }
            if (loadedReview && loadedReview.productDetail
                && loadedReview.productDetail.items[0]
                && loadedReview.productDetail.items[0].sku === sku) return loadedReview;
