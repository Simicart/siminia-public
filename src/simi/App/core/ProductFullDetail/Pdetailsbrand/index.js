...
const Pdetailsbrand = props => {
    ...
    const { brandInfo, brandInfoLoading, derivedErrorMessage } = useBrandInfo({
        url_key: url_key
    });
    ...
    if (brandInfoLoading) return fullPageLoadingIndicator;

    if (derivedErrorMessage)
        return <div className={classes.brandError}>{derivedErrorMessage}</div>;

    if (!items || !items[0].mpbrand || !items[0].mpbrand.url_key) return '';
    ...
    return (
        <section className={classes.pdetailsBrandSection}>
            ...
            />
            <Link
                to={resourceUrl(`/brands/${mpbrand.url_key}.html`)}
                className={classes.pdetailsBrandInfo}
            >
                <div
                    style={{
