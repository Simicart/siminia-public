...
import defaultClass from './Dropdown.module.css';
...
export const _Dropdown = props => {
    ...
    const optionComponents = (item.dropdown_value || []).map(v => {
    ...
    const { handleSelected, fieldName } = useBaseInput({
        ...
        defaultValue: item.required
            ? getFirstInArr(optionComponents, {}).value
            : undefined
    });
