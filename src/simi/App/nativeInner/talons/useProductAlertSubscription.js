...
const addPriceSubGuestMutation = gql`
    mutation($email: String!, $sku: String!) {
        MpProductAlertNotifyPriceDrops(
            input: { email: $email, productSku: $sku }
        ) {
            customer_email
        }
    }
...
export const useProductAlertSubscription = props => {
    ...
    const [
        addPriceDropNotificationSignedIn,
        {
    ...
    const {
        ...
        error: errorConfig
    } = useQuery(productAlertConfigQuery, {
        variables: {
            sku: sku
        }
    });
    ...
    const {
        data: priceAlertData,
        loading: priceAlertLoading,
        error: priceAlertError
    } = useQuery(generalAlertConfig);
        ...
        if (isSignedIn) {
            ...
            setPopUpData({
                ...
                callback: async email => {
                    setGlobalLoading(true);
                    return addPriceDropNotificationGuest({
    ...
    const shouldShowPriceSubscription =
        ...
        !errorConfig &&
        configData &&
        configData.products.items[0]['mp_productalerts_price_alert'] !== '0';

    const shouldShowStockSubscription =
        !!configData &&
        !errorConfig &&
        configData &&
        configData.products.items[0]['mp_productalerts_stock_notify'] !== '0' &&
