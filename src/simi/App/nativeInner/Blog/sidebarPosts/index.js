...
const PostItem = props => {
    const { name, image, publish_date, url_key } = props.item;
    return (
        <div className={classes.sidebarPostItem}>
            {image ? (
                <div className={classes.sidebarPostItemImage}>
            ...
            <div className={classes.sidebarPostItemInfo}>
                <Link
                    className={classes.sidebarPostItemName}
                    to={`/blog/post/${url_key}.html`}
                >
                    {name}
                </Link>
                <div className={classes.sidebarPostItemDate}>
                    {publish_date}
                </div>
            </div>
        </div>
...
const SidebarPosts = props => {
    ...
    const popPosts = useMemo(() => {
        if (popData && popData.mpBlogPosts && popData.mpBlogPosts.items) {
            return popData.mpBlogPosts.items.map(item => (
                <PostItem item={item} key={item.url_key} />
            ));
        }
        return [];
    }, [latestData]);
    ...
    const latestPosts = useMemo(() => {
        if (
            latestData &&
            latestData.mpBlogPosts &&
            latestData.mpBlogPosts.items
        ) {
            return latestData.mpBlogPosts.items.map(item => (
                <PostItem item={item} key={item.url_key} />
            ));
        }
        return [];
            ...
            <div className={classes.tabsHeaders}>
                <div
                    className={`${classes.tabsHeader} ${
                        tab === 'pop' ? classes.active : classes.inactive
                    }`}
                    onClick={() => setTab('pop')}
                >
                    {formatMessage({
                ...
                <div
                    className={`${classes.tabsHeader} ${
                        tab === 'latest' ? classes.active : classes.inactive
                    }`}
