...
const Optionlabel = props => {
    const {classes, title, item, type_id, raw = false} = props;
    ...
    const renderBothPrices = (included, excluded) => {
        return (
            <div style={style} className={classes['label-option-text']}>
                ...
                <span
                    className={classes['label-option-price']}
                    style={priceStyle}
                >
                    {symbol}
                ...
                <span
                    style={{...{margin: '0 10px'}, ...priceStyle}}
                    className={classes['label-option-price']}
                >
                    ({`${formatMessage({id: 'Excl. Tax'})}`}{' '}
                    {formatPrice(excluded)})
                </span>
            </div>
        );
    };
    ...
    if (
        ...
        if (
            ...
            item.price_including_tax.price
        ) {
            if (
                parseInt(merchantTaxConfig.tax_display_type) === 3 &&
                item.price_excluding_tax.price !==
                item.price_including_tax.price
            ) {
                returnedLabel = renderBothPrices(
                    item.price_including_tax.price,
                    item.price_excluding_tax.price
                );
            } else {
