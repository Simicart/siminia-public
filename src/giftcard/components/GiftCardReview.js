...
import { changeValueStar, changePriceStar, changeRatingStar } from '../functions/gift-card-review/StarHandle'
...
const GiftCardReivew = ({ giftCardData }) => {
    ...
    return (
        <div className='gift-card-review-wrapper'>
            <h1 style={{fontSize: 24, fontWeight: 'bold'}}>You're reviewing: <span style={{ fontWeight: 300 }}>{giftCardData.products.items[0].name}</span></h1>
                    ...
                    <div className='gift-card-star-wrapper'>
                        <div style={{ display: 'flex', flexDirection: 'row' }}>
                            {value.map((element, index) =>
                                <Star key={index} onClick={() => changeValueStar(index, setValue, setValueIndex)} className={`gift-card-star-${element}`} size={40}></Star>
                            )}
                        </div>
                    ...
                    <div className='gift-card-star-wrapper'>
                        <div style={{ display: 'flex', flexDirection: 'row' }}>
                            {price.map((element, index) =>
                                <Star key={index} onClick={() => changePriceStar(index, setPrice, setPriceIndex)} className={`gift-card-star-${element}`} size={40}></Star>
                            )}
                        </div>
                        <div style={{ marginTop: 20 }}>
                    ...
                    <div className='gift-card-star-wrapper'>
                        <div style={{ display: 'flex', flexDirection: 'row' }}>
                            {rating.map((element, index) =>
                                <Star key={index} onClick={() => changeRatingStar(index, setRating, setRatingIndex)} className={`gift-card-star-${element}`} size={40}></Star>
                            )}
                        </div>
                        <div style={{ marginTop: 20 }}>
            ...
            <div style={{marginBottom: 20}}>
                ...
                <input className='gift-card-nickname-input' onChange={(e) => setNickName(e.target.value)}
                    onFocus={() => { document.querySelector('.gift-card-nickname-input').style.boxShadow = '0 0 10px #206dac' }}
                    onBlur={() => { document.querySelector('.gift-card-nickname-input').style.boxShadow = 'none' }}></input>
            ...
            <div style={{marginBottom: 20}}>
                ...
                <input className='gift-card-title-input' onChange={(e) => setTitle(e.target.value)}
                    onFocus={() => { document.querySelector('.gift-card-title-input').style.boxShadow = '0 0 10px #206dac' }}
                    onBlur={() => { document.querySelector('.gift-card-title-input').style.boxShadow = 'none' }}></input>
            ...
            <div>
                ...
                <textarea className='gift-card-details-input' onChange={(e) => setDetails(e.target.value)}
                    onFocus={() => { document.querySelector('.gift-card-details-input').style.boxShadow = '0 0 10px #206dac' }}
                    onBlur={() => { document.querySelector('.gift-card-details-input').style.boxShadow = 'none' }}></textarea>
                {detailsError && (<p style={{ color: 'red', fontSize: 12, marginTop: 5 }}>This is a required field.</p>)}
            </div>
            <div className={submitButton === 'Submitting...' ? 'gift-card-submit-review-loading-button' : 'gift-card-submit-review-button'}>
