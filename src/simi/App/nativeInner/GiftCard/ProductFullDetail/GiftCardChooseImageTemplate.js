...
const GiftCardChooseImageTemplate = props => {
	...
	const nextTemplateImage = () => {
		...
		if(
			translateOwlStage < initialTransDistance - (images.length-1.2)*owlStageStep
			&& translateOwlStage > initialTransDistance - (images.length-0.8)*owlStageStep
		) {
			return;
		} else if(
			translateOwlStage < initialTransDistance - (images.length-2.2)*owlStageStep
			&& translateOwlStage > initialTransDistance - (images.length-1.8)*owlStageStep
		) {
		...
		<React.Fragment>
				...
				<div className={classes['owl-stage-outer']}>
					...
					>
						{images.map((image,i) => {
							...
							return (
								<div className={classes['owl-item']} style={{width: `${itemWidth}px`}} key={i+1}>
									<div className={classes.image}>
										<div 
											className={classes['template-image-wrapper'] + ' ' + active}
				...
				{images.length > 3 && 
					<div className={classes['owl-nav']}>
						<button 
							type="button" 
							role="presentation" 
							className={`${classes["owl-prev"]} ${disabledPrev ? classes.disable : ''}`} 
							onClick={prevTemplateImage}
						><i className="fa fa-chevron-left"></i></button>
						<button 
							type="button" 
							role="presentation" 
							className={`${classes["owl-next"]} ${disabledNext ? classes.disable : ''}`} 
			...
			{ canUpload && 	
				<div className={classes['giftcard-template-upload']}>
	                ...
	                <div id={classes["giftcard-template-upload-image"]} className={classes["giftcard-template-upload-image"]}>
	               		{ uploadedImages.length ?
	               			uploadedImages.map((image, i) => {
								...
								if(activeImage === images.length + i) {
	               				...
	               				return (
			               			<div className={classes.image} key={i+1}>
										<div 
											className={classes['template-image-wrapper'] + ' ' + active}
											onClick={() => setActiveImage(images.length + i)}
										>
											<img className={classes['template-image']} src={image} alt='Gift Card Image'/>
										</div>
									</div>
								)
							})
	                	...
	                	<div className={classes["image"] + ' ' + classes['item-template']}>
	                        <div className={classes['fileinput-button']}>
	                        	{ !uploadGcImageLoading ?
	                        		<i className={`fa fa-camera + ${classes['image-placeholder']}`}></i>
	                        		: <img src={Loading} alt='loading' style={{width: '100%'}}/>
	                        	}
	                        	<input id={classes.fileupload} type='file' name='image' onChange={handleUploadImage}/>
	                        </div>
	                    </div>
	                </div>
				</div>
