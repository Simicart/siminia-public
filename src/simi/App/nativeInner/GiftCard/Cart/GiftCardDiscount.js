...
const GiftCardDiscount = props => {
    ...
    const [isRemoveCodeMsgDisplayed, setIsRemoveCodeMsgDisplayed] = useState(false)
    const [isSetCreditMsgDisplayed, setIsSetCreditMsgDisplayed] = useState(false)
    ...
    const displayLoading = setCodeLoading || setCreditLoading || removeCodeLoading || checkCodeLoading
    ...
    const handleSetCode = (code) => {
        ...
        setGiftCardCode({ variables: { cartId: cartId, code: code.toUpperCase().trim() }})
            .then(res => void 0)
            .catch(err => void 0)
    ...
    const handleRemoveCode = (code) => {
        removeGiftCardCode({variables: { cartId: cartId, code: code.toUpperCase().trim() }})
            .then(res => void 0)
            .catch(err => void 0)
    ...
    const handleSetCredit = (e) => {
        ...
        if(isNaN(value) || value.length == 0 || value < 0) {
            e.target.value = 0
            setCreditValue(0)
            setGiftCardCredit({ variables: { cartId: cartId, amount: e.target.value }})
    ...
    const removeCodeMessage = (
        <div className={classes['message-container']}>
            { (isRemoveCodeMsgDisplayed && !removeCodeErrorMessage && !removeCodeLoading) && 
                <div className={classes['message-success']}>
                    <Icon.Check className={classes['message-icon']}/>
    ...
    const setCreditMessage = (
        <div className={classes['message-container']}>
            { (isSetCreditMsgDisplayed && !setCreditErrorMessage && !setCreditLoading) && 
                <div className={classes['message-success']}>
                    <Icon.Check className={classes['message-icon']}/>
                    Set gift card credit successfully.
                </div>
            }
            { (isSetCreditMsgDisplayed && setCreditErrorMessage  && !setCreditLoading) && 
                <div className={classes['message-error']}>
            ...
            <div className={classes.content}>
                ...
                <div className={classes['giftcard-apply']}>
                    <div className={classes['apply-gift-card-field']}>
                        <input type='text' className={classes['input-code']} placeholder='Enter gift card code' value={gcCode} onChange={onCodeChange}/>
                        <button 
                            ...
                            giftCardUsed.map(data => {
                                return (
                                    <div className={classes['gift-card-used-item']}>
                                        <a 
                                            href={void(0)} 
                                            onClick={() => handleRemoveCode(data.code)}
                                        ><Icon.X className={classes['close-icon']}/></a>
                                        <span className={classes['gift-card-used-item-code']}>{`${data.code} (-$${data.amount})`}</span>
                                    </div>
                                )
                            })
                        }
                    ...
                    <div className={classes['gift-card-saved-list']}>
                        <div className={classes['saved-list-title']} onClick={() => setIsSavedListClosed(!isSavedListClosed)}>
                            Choose from saved list
                            { !isSavedListClosed && <Icon.ChevronDown className={classes.icon}/> }
                            { isSavedListClosed && <Icon.ChevronUp className={classes.icon}/> }
                        </div>
                        <div className={classes['saved-list-content-container']} style={{display: isSavedListClosed ? 'inline-flex' : 'none'}}>
                            <div className={classes['saved-list-content']}>
                                <select 
                                    id={classes['gift-card-saved-list']} 
                                    onChange={(e) => {
                                        if(e.target.value != 0) handleSetCode(e.target.value)
                                    }}
                                >
                            ...
                            <button className={classes['action']}><span>Apply</span></button>
                        </div>
                    </div>
                    { (checkCodeData && checkCodeData.mpGiftCardCheckCode) &&
                        <div className={classes['gift-card-check-code']} style={{display: isInformationDisplayed ? 'block' : 'none'}}>
                            <div className={classes['fieldset']}>
                                <div className={classes['field']}>
                                    <label className={classes['label']}><span>Status</span></label>
                                    <div className={classes['control']}><span>{checkCodeData.mpGiftCardCheckCode.status_label}</span></div>
                                </div>
                                <div className={classes['field']}>
                                    <label className={classes['label']}><span>Balance</span></label>
                                    <div className={classes['control']} dangerouslySetInnerHTML={{__html: checkCodeData.mpGiftCardCheckCode.balance_formatted}}/>
                                </div>
                                <div className={classes['field']}>
                                    <label className={classes['label']}><span>Expired At</span></label>
                                    <div className={classes['control']}><span>{checkCodeData.mpGiftCardCheckCode.expired_at}</span></div>
                                </div>
                            </div>
                        </div>
                    }
                    <button className={classes['action']} onClick={onCheckCodeClick}><span>Check</span></button>
                </div>
                { maxUsed ?
                    <React.Fragment>
                        <div className={classes['credit-title']} onClick={() => setIsApplyCreditClosed(!isApplyCreditClosed)}>
                            Use Gift Credit
                            { !isApplyCreditClosed && <Icon.ChevronDown className={classes.icon}/> }
                            { isApplyCreditClosed && <Icon.ChevronUp className={classes.icon}/> }
                        </div>
                        <div className={classes.content + ' ' + classes.credit} style={{display: isApplyCreditClosed ? 'flex' : 'none'}}>
