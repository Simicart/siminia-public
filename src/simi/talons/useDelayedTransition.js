...
const useDelayedTransition = () => {
    ...
    useEffect(() => {
        globalThis.handleRouteChangeConfirmation = async (message, proceed) => {
            ...
            //don't need to get urlresolver on pbPages
            const smCurrentPathname = message.replace(DELAY_MESSAGE_PREFIX, '');
            if (smCurrentPathname && window.smPbPagesByToken) {
                try {
                    const { items } = window.smPbPagesByToken.data.spb_page;
                    let isPbPage = false;
                    items.map(pbPage => {
                        if (pbPage.url_path === smCurrentPathname)
                            isPbPage = true;
                    });
