...
const SimpleHeader = props => {
    ...
    const titleObj = customTitle ? customTitle : (
        <span className={`header-title ${defaultClasses1['title-zone']}`}>{titleText}</span>
    )

    const leftIcon = canGoBack ? (
        <span className={`main-header-icon ${defaultClasses1["back-zone"]}`}
