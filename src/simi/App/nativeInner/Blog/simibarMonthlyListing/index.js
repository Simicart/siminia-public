...
const SimibarMonthlyListing = props => {
    const { data: archiveListData } = useQuery(GET_BLOG_ARCHIVE, {
    ...
    if (
        archiveListData &&
        archiveListData.mpBlogMonthlyArchive &&
        archiveListData.mpBlogMonthlyArchive.items
    ) {
        ...
        return (
            <div className={classes.listRoot}>
                <div className={classes.listHeader}>
                    {formatMessage({
                        id: 'monthlyArchive',
                        defaultMessage: 'Monthly Archive'
                    })}
                </div>
                <div className={classes.listItems}>
                    {archiveItems.map((archiveItem, index) => {
                        try {
                            ...
                            if (
                                archiveData &&
                                archiveData.getFullYear() &&
                                archiveData.getMonth()
                            ) {
                                return (
                                    <Link
                                        className={classes.listItem}
                                        to={`/blog/month/${archiveData.getFullYear()}-${
                                            archiveData.getMonth() + 1 > 9
                                                ? ''
                                                : '0'
                                        }${archiveData.getMonth() + 1}.html`}
                                        key={archiveItem.label}
                                    >
                                        {calendarIcon} {archiveItem.label} (
                                        {archiveItem.quantity})
                                    </Link>
                                );
                            }
                        } catch (err) {}
