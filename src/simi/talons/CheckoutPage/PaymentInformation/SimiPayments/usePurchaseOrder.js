...
export const usePurchaseOrder = props => {
    ...
    const submitPaymentInfo = useCallback(
        ...
        [setSelectedPaymentMethod, cartId, paymentCode, purchaseOrderNumber]
    );
    ...
    useEffect(() => {
        try {
            if (shouldSubmit) {
                ...
                let hasErrors = Object.keys(formState.errors).length;
                if (!hasErrors) {
                    if (!purchaseOrderNumber || hasLengthAtLeast(String(purchaseOrderNumber).trim(), null, 1)) {
                        hasErrors = true
                        showToastMessage(Identify.__('Please fill your Purchase Order Number'))
                    }
                }
                setStepNumber(1);
